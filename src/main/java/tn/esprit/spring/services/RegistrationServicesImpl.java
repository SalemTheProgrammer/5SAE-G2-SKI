...
public class RegistrationServicesImpl  {
    ...
    @Transactional
    public Registration addRegistrationAndAssignToSkierAndCourse(Registration registration, Long numSkieur, Long numCours, LocalDate startDateInclusive) {
        Skier skier = skierRepository.findById(numSkieur).orElse(null);
        Course course = courseRepository.findById(numCours).orElse(null);

        // Ensure startDateInclusive is not null
        if (skier == null || course == null || startDateInclusive == null) {
            log.error("Skier, Course or Start Date cannot be null");
            return null;
        }
